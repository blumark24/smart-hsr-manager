...
// Phase 03B/06A deliberately evolved service editing into independent
// Field, Mobility, and Lands entitlements on one identity. Each enabled
// product has one role pill selected and submitServiceChange persists the
// complete explicit state, preventing accidental carry-over.
test('Edit user services: independent product toggles expose explicit role options', () => {
  const source = read('manager.html');
  assert.match(source, /svcFieldRoleOptions:/);
  assert.match(source, /svcMobilityRoleOptions:/);
  assert.match(source, /svcLandsRoleOptions:/);
  assert.match(source, /async submitServiceChange\(\)/);
