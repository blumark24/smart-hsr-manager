...
const sha=file=>crypto.createHash('sha256').update(fs.readFileSync(path.resolve(root,file))).digest('hex');const valid={...example,assignmentV2Enabled:true};const activate=(contract=valid,overrides={})=>evaluateStagingActivation({contract,explicitOverride:true,hostname:'preview-smart-hsr.test',firebaseProjectId:'demo-smart-hsr-staging',candidateRulesHash:candidateHash,...overrides});
