...
const {evaluateStagingActivation,validateStagingEnvironmentContract}=require('../platform/staging/staging-environment-contract');const example=require('../config/staging/assignment-v2-staging.example');const seed=require('../staging/demo/assignment-v2-demo-seed');
const root=path.resolve(__dirname,'..'),candidateHash='3aa6bcdc6c4659f5f417bcec24fed6f3ec8cda3a724a789c39ad0634406c088a',legacyHash='2d36ab1ede72054e1b197a0126502cbbfc49f35e983c2aeeb8fa8d167e8784e7',bundleHash='83214ea9929b6ecad4783f4dd8d75422fab90aa5d5005cecc7738132995f1299';
const sha=file=>crypto.createHash('sha256').update(fs.readFileSync(path.resolve(root,file),'utf8').replace(/\r\n/g,'\n'),'utf8').digest('hex');const valid={...example,assignmentV2Enabled:true};const activate=(contract=valid,overrides={})=>evaluateStagingActivation({contract,explicitOverride:true,hostname:'preview-smart-hsr.test',firebaseProjectId:'demo-smart-hsr-staging',candidateRulesHash:candidateHash,...overrides});
test('missing staging config is denied',()=>assert.equal(evaluateStagingActivation({}).allowed,false));
test('unknown hostname is denied',()=>assert.equal(activate(valid,{hostname:'unknown.example'}).allowed,false));
test('unknown project id is denied',()=>assert.equal(activate(valid,{firebaseProjectId:'unknown-project'}).allowed,false));
test('assignmentV2Enabled false is denied and remains the example default',()=>{assert.equal(example.assignmentV2Enabled,false);assert.equal(activate(example).allowed,false);});
test('demoDataOnly false is denied',()=>assert.equal(activate({...valid,demoDataOnly:false}).allowed,false));
test('rules hash mismatch is denied',()=>assert.equal(activate(valid,{candidateRulesHash:'0'.repeat(64)}).allowed,false));
test('missing explicit override is denied',()=>assert.equal(activate(valid,{explicitOverride:false}).allowed,false));
test('valid staging contract is allowed',()=>{assert.equal(validateStagingEnvironmentContract(valid).allowed,true);assert.equal(activate().allowed,true);});
