...
function legacyRow(record, mobile=false) {
  ...
  return `<tr class="ucv2-legacy"><td><div class="ucv2-person"><span class="ucv2-avatar">${esc(initials(u.name||u.email))}</span><div><b>${esc(u.name||'حساب قديم')}</b><small>${esc(u.uid||'—')}</small></div></div></td><td class="ucv2-email">${esc(u.email||'—')}</td><td colspan="6"><span class="ucv2-legacy-note">حساب قديم غير مرتبط بسجل موظف — يحتاج ربطًا آمنًا قبل التعديل.</span></td><td><span class="ucv2-chip warn">حساب قديم</span></td><td><button class="ucv2-edit-btn" disabled aria-disabled="true" title="يتطلب ربطًا آمنًا">—</button></td></tr>`;
}
function contractorRow(record,mobile=false){
  ...
  return `<tr class="ucv2-contractor-row"><td><div class="ucv2-person"><span class="ucv2-avatar">${esc(initials(c.companyName||c.contactName))}</span><div><b>${esc(c.companyName||'شركة متعاقدة')}</b><small>${esc(c.contractNumber||'—')}</small></div></div></td><td class="ucv2-email">${esc(c.email||'—')}</td><td>${esc(c.administration||'إدارة الحصر الميداني')}</td><td>${esc(c.section||'التشوه البصري')}</td><td>${esc(c.contactName||'ممثل الشركة')}</td><td><div class="ucv2-products"><span class="ucv2-chip product field">الحصر</span></div></td><td><span class="ucv2-muted">—</span></td><td><span class="ucv2-muted">—</span></td><td><span class="ucv2-chip ${c.active===false?'danger':stateClass}">${c.active===false?'الحساب موقوف':esc(stateLabel)}</span></td><td><button class="ucv2-edit-btn" disabled aria-disabled="true" title="إدارة الشركة من سجل المتعاقدين">—</button></td></tr>`;
}
function employeeRow(record, mobile=false) {
  const e=record.employee, status=accountStatus(e), id=esc(e.employeeId);
  if (mobile) return `<article class="ucv2-mobile-card" data-employee-id="${id}"><div class="ucv2-person"><span class="ucv2-avatar">${esc(initials(e.name))}</span><div><b>${esc(e.name||'موظف')}</b><small>${esc(e.email||e.employeeRef||'—')}</small></div></div><div class="ucv2-mobile-meta">${statusBadge(status)}<span>${esc(institutionalSection(e)||'بدون قسم')}</span></div><div class="ucv2-products">${productChips(e)}</div><div class="ucv2-mobile-actions">${editIconButton(id,'تعديل الموظف')}</div></article>`;
  return `<tr data-employee-id="${id}" tabindex="0"><td><div class="ucv2-person"><span class="ucv2-avatar">${esc(initials(e.name))}</span><div><b>${esc(e.name||'موظف')}</b><small>${esc(e.employeeRef||'—')}</small></div></div></td><td class="ucv2-email">${esc(e.email||'—')}</td><td>${esc(institutionalAdministration(e))}</td><td>${esc(institutionalSection(e))}</td><td>${esc(e.jobTitle||'—')}</td><td><div class="ucv2-products">${productChips(e)}</div></td><td>${mobilityRoleMarkup(e)}</td><td>${vehicleBadge(e)}</td><td>${statusBadge(status)}</td><td class="ucv2-edit-cell">${editIconButton(id,'تعديل الموظف')}</td></tr>`;
}
