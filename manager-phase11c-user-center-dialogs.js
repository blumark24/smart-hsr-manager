...
let sp=c.querySelector('.savep');sp.onclick=async()=>{let m=c.querySelector('.mp'),name=U.clean(c.querySelector('#edit-name').value),nextEmail=U.clean(c.querySelector('#edit-email').value).toLowerCase(),currentEmail=U.clean(e.email).toLowerCase();if(!name)return m.className='msg er mp',m.textContent='الاسم مطلوب.';if(nextEmail&&!/^[^\\s@]+@[^\\s@]+\\.[^\\s@]+$/.test(nextEmail))return m.className='msg er mp',m.textContent='البريد غير صالح.';sp.disabled=true;try{if(linked&&nextEmail&&nextEmail!==currentEmail){await U.post('/api/admin/employees',{action:'changeLoginEmail',employeeId:e.employeeId,email:nextEmail,confirmEmail:nextEmail});e.email=nextEmail;}let q={action:'updateProfile',employeeId:e.employeeId,name,employeeRef:U.clean(c.querySelector('#edit-ref').value),phone:U.clean(c.querySelector('#edit-phone').value),jobTitle:U.clean(c.querySelector('#edit-title').value),employmentStatus:c.querySelector('#edit-emp').value};if(!linked)q.email=nextEmail;await U.post('/api/admin/employees',q);e.employeeRef=q.employeeRef;e.phone=q.phone;e.jobTitle=q.jobTitle;e.employmentStatus=q.employmentStatus;if(!linked)e.email=nextEmail;U.stale();m.className='msg ok mp';m.textContent='تم حفظ بيانات الموظف والبريد.'}catch(er){m.className='msg er mp';m.textContent=er.reason==='reauthentication_required'?'لحماية الحساب، سجّل الخروج ثم ادخل من جديد واحفظ البريد.':U.why(er.reason||er.message)}finally{sp.disabled=false}};
