...
let sp=c.querySelector('.savep');sp.onclick=async()=>{let m=c.querySelector('.mp'),name=U.clean(c.querySelector('#edit-name').value);if(!name)return m.className='msg er mp',m.textContent='الاسم مطلوب.';sp.disabled=true;try{let q={action:'updateProfile',employeeId:e.employeeId,name,employeeRef:U.clean(c.querySelector('#edit-ref').value),phone:U.clean(c.querySelector('#edit-phone').value),jobTitle:U.clean(c.querySelector('#edit-title').value),employmentStatus:c.querySelector('#edit-emp').value};if(!linked)q.email=U.clean(c.querySelector('#edit-email').value);await U.post('/api/admin/employees',q);U.stale();m.className='msg ok mp';m.textContent='تم حفظ بيانات الموظف.'}catch(er){m.className='msg er mp';m.textContent=U.why(er.reason||er.message)}finally{sp.disabled=false}};
