...
U.profile=async e=>{let d=(U.cache&&Array.isArray(U.cache.employees)&&U.cache.employees.length)?U.cache:await U.dir(false);let cached=d.employees.find(x=>x.employeeId===e.employeeId);e={...(cached||{}),...e};let linked=!!e.authUid,role=U.inst(e),mans=U.managerOptions(d.employees,role,e.administration||'',e.department||'',e.employeeId),body=`${U.hierarchy()}<div class="tabs"><button class="tab on" data-t="p">البيانات الأساسية</button><button class="tab" data-t="o">التنظيم</button><button class="tab" data-t="a">الحساب والأمان</button><button class="tab" data-t="r">المنتجات والصلاحيات</button><button class="tab" data-t="h">السجل</button></div><div class="pane" data-id="p"><div class="sec"><div class="grid">${U.input('الاسم','edit-name',e.name,{w:true})}${U.input('الرقم الوظيفي','edit-ref',e.employeeRef||'')}${U.input('الجوال','edit-phone',e.phone||'',{type:'tel'})}${U.input('البريد','edit-email',e.email||'',{type:'email',ac:'email'})}${U.input('المسمى','edit-title',e.jobTitle||'')}${U.sel('الحالة الوظيفية','edit-emp',e.employmentStatus||'active',[['active','على رأس العمل'],['inactive','غير نشط']])}</div></div><div class="act"><button class="btn pr savep">حفظ البيانات</button></div><div class="msg mp"></div></div><div class="pane" data-id="o" hidden><div class="sec"><div class="grid">${U.input('الإدارة','edit-admin',e.administration||'')}${U.input('القسم','edit-dept',e.department||'')}${U.sel('الدور المؤسسي','inst-role',role,[['manager','مدير البلدية — من حساب المؤسسة',true],['general_supervisor','المشرف العام'],['department_head','رئيس قسم'],['employee','موظف']])}${U.sel('المدير المباشر','edit-manager',e.directManagerEmployeeId||'',mans,{w:true})}</div></div><div class="act"><button class="btn pr saveo">حفظ التنظيم</button></div><div class="msg mo"></div></div><div class="pane" data-id="a" hidden><div class="sec">${linked?`<div class="note">الحساب: ${U.esc(e.accountStatus||'ACTIVE')} · ${U.esc(e.email||'')}</div><div class="act"><button class="btn pwbtn">تعديل كلمة المرور</button><button class="btn ${e.accountStatus==='SUSPENDED'?'pr':'bad'} tog">${e.accountStatus==='SUSPENDED'?'تفعيل الحساب':'إيقاف الحساب'}</button></div>`:`<div class="grid">${U.input('بريد الدخول','act-email',e.email||'',{type:'email'})}${U.input('كلمة المرور','act-pw','',{type:'password',ac:'new-password'})}${U.input('تأكيد كلمة المرور','act-pw2','',{type:'password',ac:'new-password'})}</div><div class="note">حدد الدور والصلاحيات من تبويبي «التنظيم» و«الصلاحيات والخدمات» ثم أنشئ حساب الدخول.</div><div class="act"><button class="btn pr activate">إنشاء حساب الدخول</button></div>`}</div><div class="msg ma"></div></div><div class="pane" data-id="r" hidden>${U.permissions(role,e)}${linked?'<div class="act"><button class="btn pr saver">حفظ الصلاحيات والخدمات</button></div>':'<div class="note">يمكن تجهيز الصلاحيات الآن، وستُطبق عند إنشاء حساب الدخول.</div>'}<div class="msg mr"></div></div><div class="pane" data-id="h" hidden><div class="sec hist"><div class="msg">جاري تحميل التكليفات...</div></div></div>`,{c}=U.shell('iuc-profile','ملف الموظف',`${e.name} · الدور المؤسسي: ${role==='general_supervisor'?'المشرف العام':role==='department_head'?'رئيس قسم':'موظف'}`,body);U.bind(c);const refreshProfileManagers=()=>{let s=c.querySelector('#edit-manager');if(!s)return;let current=s.value,opts=U.managerOptions(d.employees,c.querySelector('#inst-role')?.value||role,c.querySelector('#edit-admin')?.value||'',c.querySelector('#edit-dept')?.value||'',e.employeeId);s.innerHTML=opts.map(x=>`<option value="${U.esc(x[0])}">${U.esc(x[1])}</option>`).join('');if(opts.some(x=>String(x[0])===String(current)))s.value=current};c.querySelector('#edit-dept').oninput=()=>{U.dest(c);refreshProfileManagers()};c.querySelector('#edit-admin').oninput=refreshProfileManagers;c.querySelector('#inst-role')?.addEventListener('change',refreshProfileManagers);refreshProfileManagers();c.querySelectorAll('.tab').forEach(t=>t.onclick=()=>{c.querySelectorAll('.tab').forEach(x=>x.classList.toggle('on',x===t));c.querySelectorAll('.pane').forEach(p=>p.hidden=p.dataset.id!==t.dataset.t)});
...
let so=c.querySelector('.saveo');so.onclick=async()=>{let m=c.querySelector('.mo');so.disabled=true;try{await U.post('/api/admin/employees',{action:'transfer',employeeId:e.employeeId,administration:U.clean(c.querySelector('#edit-admin').value),department:U.clean(c.querySelector('#edit-dept').value),institutionalRole:c.querySelector('#inst-role').value,directManagerEmployeeId:c.querySelector('#edit-manager').value||''});e.institutionalRole=c.querySelector('#inst-role').value;U.stale();m.className='msg ok mo';m.textContent='تم حفظ التغيير وإضافته لسجل النقل.'}catch(er){m.className='msg er mo';m.textContent=U.why(er.reason||er.message)}finally{so.disabled=false}};
